...
const ImportPage = ({ evaluasi }: ImportProps) => {
    ...
    const onSubmit = async (values: any[]) => {
        ...
        const dataSorted = values.sort((a, b) => {
            const levelOrder: any = { "kelompok": 0, "kriteria": 1, "iku": 2 };
            return levelOrder[a.level] - levelOrder[b.level];
        });
        const dataFiltered = dataSorted.filter((item) => {
            if ((item.level === "kelompok") || (item.level === "kriteria") || (item.level === "iku")) {
                return item
            }
        })
        const value = {
        ...
        try {
            ...
            toast.success("Variabel KKE dibuat!")
            router.refresh()
        } catch {
        ...
        <>
            ...
            <Dialog>
                <DialogTrigger asChild>
                    <Button>Import variabel KKE</Button>
                </DialogTrigger>
